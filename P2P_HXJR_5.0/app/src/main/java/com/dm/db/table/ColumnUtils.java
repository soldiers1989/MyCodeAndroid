...
import com.dm.db.annotation.Check;
import com.dm.db.annotation.Column;
import com.dm.db.annotation.Finder;
import com.dm.db.annotation.Foreign;
import com.dm.db.annotation.Id;
import com.dm.db.annotation.NotNull;
import com.dm.db.annotation.Transient;
import com.dm.db.annotation.Unique;
...
public class ColumnUtils {
	...
	public static Method getColumnGetMethod(Class<?> entityType, Field field) {
		...
		if (getMethod == null) {
            String methodName =
                "get" + fieldName.substring(0, 1).toUpperCase(Locale.getDefault())
					+ fieldName.substring(1);
			try {
				getMethod = entityType.getDeclaredMethod(methodName);
			} catch (NoSuchMethodException e) {
				DMLog.d(methodName + " not exist");
	...
	public static Class<?> getForeignEntityType(
			com.dm.db.table.Foreign foreignColumn) {
	...
	public static Class<?> getFinderTargetEntityType(
			com.dm.db.table.Finder finderColumn) {
	...
	private static Method getBooleanColumnGetMethod(Class<?> entityType,
        ...
        String methodName = "is" + fieldName.substring(0, 1).toUpperCase(Locale.getDefault())
				+ fieldName.substring(1);
	...
	private static Method getBooleanColumnSetMethod(Class<?> entityType,
		...
		if (isStartWithIs(field.getName())) {
            methodName = "set" + fieldName.substring(2, 3).toUpperCase(Locale.getDefault())
					+ fieldName.substring(3);
		} else {
            methodName = "set" + fieldName.substring(0, 1).toUpperCase(Locale.getDefault())
					+ fieldName.substring(1);
		}
		try {
