...
public class FanProgressBar extends View
{
	...
	public FanProgressBar(Context context, AttributeSet attrs, int defStyle)
	{
		...
		bgRoundWidth = mTypedArray.getDimension(R.styleable.FanProgressBar_bgWidth, 5);
		fgRoundWidth = mTypedArray.getDimension(R.styleable.FanProgressBar_fgWidth, 5);
	...
	protected void onDraw(Canvas canvas)
	{
		...
		int radius = (int)(centre - tempWidth / 2); // 圆环的半径
		...
		switch (style)
		{
			case STROKE:
			{
				...
				canvas.drawArc(mOval, -210, 240 * progress / max, false, paint); // 根据进度画圆弧
				break;
			}
			case FILL:
			{
				...
				if (progress != 0)
					canvas.drawArc(mOval, -210, 360 * progress / max, true, paint); // 根据进度画圆弧
				break;
			}
		}
	...
	public synchronized void setProgress(int progress)
	{
		...
		if (progress <= max)
		{
			...
			if (progress < 31)
			{
				//				roundProgressColor = Color.parseColor("#ff35b8fc");
			}
			else if (progress < 71)
			{
				//				roundProgressColor = Color.parseColor("#ff35b8fc");
	...
	/**
	 * @return 返回 fgRoundWidth
	 */
	public float getFgRoundWidth()
	{
		return fgRoundWidth;
	}
	
	/**
	 * @param 对fgRoundWidth进行赋值
	 */
	public void setFgRoundWidth(float fgRoundWidth)
	{
		this.fgRoundWidth = fgRoundWidth;
	}
	
	/**
	 * @return 返回 bgRoundWidth
	 */
	public float getBgRoundWidth()
	{
		return bgRoundWidth;
	}
	
	/**
	 * @param 对bgRoundWidth进行赋值
	 */
