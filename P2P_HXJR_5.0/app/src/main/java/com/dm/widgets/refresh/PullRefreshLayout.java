...
public class PullRefreshLayout extends ViewGroup {
	...
	public PullRefreshLayout(Context context, AttributeSet attrs) {
        ...
        mRefreshView.setScaleType(ScaleType.CENTER_INSIDE);
        ...
        mLoadView.setScaleType(ScaleType.CENTER_CROP);
        addView(mLoadView, 1);
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		widthMeasureSpec = MeasureSpec.makeMeasureSpec(getMeasuredWidth() - getPaddingRight()
...
//					setTargetOffsetTop(-targetY - mCurrentOffsetTop, true);
		...
		case MotionEvent.ACTION_CANCEL: {
			...
			if (overscrollTop > mTotalDragDistance && mCurrentOffsetTop > mTotalDragDistance) {
				...
				mLastDirection = RefreshMode.PULL_FROM_START;
			} else if (Math.abs(overscrollTop) > mTotalDragDistance
				&& mCurrentOffsetTop < -mTotalDragDistance) {
				setLoading(true);
				mLastDirection = RefreshMode.PULL_FROM_END;
			} else {
				mRefreshing = false;
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		mTarget.layout(left, top + mTarget.getTop(), left + width - right, top + height - bottom
			+ mTarget.getTop());
