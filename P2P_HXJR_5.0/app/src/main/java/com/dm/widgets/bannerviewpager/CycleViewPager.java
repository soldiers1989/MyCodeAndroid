...
public class CycleViewPager extends LinearLayout implements OnPageChangeListener
{
	...
	private ViewGroup indicatorLayout; // 图片轮播指示器控件
	...
	private List<AdvertisementInfo> infos;
	...
	public void init()
	{
		LayoutInflater.from(mContext).inflate(R.layout.view_cycle_viewpager_contet, this);
		viewPager = (BaseViewPager)findViewById(R.id.viewPager);
		// 滚动图片右下指示器视图
		indicatorLayout = (ViewGroup)findViewById(R.id.viewGroup);
		if (!isInEditMode())
		{
		...
		ViewPagerScroller mViewPagerScroller = new ViewPagerScroller(mContext);
		mViewPagerScroller.initViewPagerScroll(viewPager);
		
		handler = new CycleViewPagerHandler(mContext)
		{
	...
	public void setData(List<ImageView> views, List<AdvertisementInfo> list, ImageCycleViewListener listener)
	{
	 ...
	 *            默认显示位置
	 */
	public void setData(List<ImageView> views, List<AdvertisementInfo> list, ImageCycleViewListener listener, int showPosition)
	{
		...
		//					R.layout.view_cycle_viewpager_indicator, this);
	...
	private void setIndicator(int selectedPosition)
	{
		for (int i = 0; i < indicators.length; i++)
		{
			indicators[i].setBackgroundResource(R.drawable.home_banner_point_off);
		}
		if (indicators.length > selectedPosition)
			indicators[selectedPosition].setBackgroundResource(R.drawable.home_banner_point_on);
	}
	...
	public static interface ImageCycleViewListener
	{
		 ...
		 * @param imageView
		 */
		public void onImageClick(AdvertisementInfo info, int postion, View imageView);
	}
}