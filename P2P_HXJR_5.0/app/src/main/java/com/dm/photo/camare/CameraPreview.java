...
public class CameraPreview extends SurfaceView implements SurfaceHolder.Callback, AutoFocusCallback {
	...
	public void surfaceCreated(SurfaceHolder holder) {
		...
		try {
            ...
            CameraUtil.getInstance().setCameraDisplayOrientation((Activity)context, 0, camera);
		} catch (IOException e) {
			e.printStackTrace();
			// 释放手机摄像头
			camera.release();
...
//				if (p != null) {
//					// 获取相机所有支持尺寸
//					videoSiezes = p.getSupportedVideoSizes();
//					for (Camera.Size size : videoSiezes) {
//						DMLog.e(size.toString());
//					}
//				}
//
//				if (videoSiezes != null && videoSiezes.size() > 0) {
//					// 拿到一个预览宽度最小为720像素的预览值
//					Camera.Size videoSize = CameraUtil.getInstance().getPropVideoSize(videoSiezes,
//						1080);
//					video_width = videoSize.width;
//					video_height = videoSize.height;
//					// 这里第三个参数为最小尺寸 getPropPreviewSize方法会对从最小尺寸开始升序排列
//					// 取出所有支持尺寸的最小尺寸
//					Camera.Size previewSize = CameraUtil.getInstance().getPropPreviewSize(
//						p.getSupportedPreviewSizes(), video_width);
//					p.setPreviewSize(previewSize.width, previewSize.height);
//
//					Camera.Size pictrueSize = CameraUtil.getInstance().getPropPictureSize(
//						p.getSupportedPictureSizes(), video_width);
//					p.setPictureSize(pictrueSize.width, pictrueSize.height);
//
//					camera.setParameters(p);
//				} else {
//					// 这里第三个参数为最小尺寸 getPropPreviewSize方法会对从最小尺寸开始升序排列
//					// 取出所有支持尺寸的最小尺寸
//					Camera.Size previewSize = CameraUtil.getInstance().getPropPreviewSize(
//						p.getSupportedPreviewSizes(), 1080);
//					p.setPreviewSize(previewSize.width, previewSize.height);
//
//					Camera.Size pictrueSize = CameraUtil.getInstance().getPropPictureSize(
//						p.getSupportedPictureSizes(), 1080);
//					p.setPictureSize(pictrueSize.width, pictrueSize.height);
//
//					camera.setParameters(p);
//
...
//	}
	...
	public void focusOnTouch(MotionEvent event) {
		...
		Rect focusRect = Utils.calculateTapArea(mFocusView.getWidth(), mFocusView.getHeight(), 1f,
			event.getRawX(), event.getRawY(), location[0], location[0] + relativeLayout.getWidth(),
			location[1], location[1] + relativeLayout.getHeight());
		Rect meteringRect = Utils.calculateTapArea(mFocusView.getWidth(), mFocusView.getHeight(),
			1.5f, event.getRawX(), event.getRawY(), location[0],
